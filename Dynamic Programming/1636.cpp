...
int main(void){
	...
	while(++tcase, t--){
		...
		dp[0][0] = 1;
		/*
		state(i, x) = i -> no of coin till, x -> current target sum
		base case: (0, 0) -> 1
		STE: dp[i][x] = dp[i - 1][x] // don't take current coin
						+ dp[i][x - ci] // take current coin value

		ans: dp[n][x] // ways using n coins for x sum

		*/
		// accurate but gives TLE due to cache misses of [i - c[j - 1]]
		// for(int i = 0; i <= x; ++i){
		// 	for(int j = 1; j <= n; ++j){
		// 		dp[j][i] = (dp[j - 1][i] % MOD.i + (i - c[j - 1] >= 0? dp[j][i - c[j - 1]] : 0)) % MOD.i;
		// 	}
		// }

		// put the more varying and far(x) part in innner loop
		for(int j = 1; j <= n; ++j){
			for(int i = 0; i <= x; ++i){
				dp[j][i] = (dp[j - 1][i] % MOD.i + (i - c[j - 1] >= 0? dp[j][i - c[j - 1]] : 0)) % MOD.i;
			}
