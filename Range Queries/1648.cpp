...
const lli N = 2e5 + 5; // max limit of array
lli n; // array size
vector<lli> tree(4 * N); // seg tree

void build(){ // build the tree; leaves are stored in continuous nodes with indices starting with n
	for(lli i = n - 1; i > 0; --i) tree[i] = tree[i << 1] + tree[i << 1 | 1]; //eq. t[2i] + t[2i+1]
}

void update(lli p, lli val){ // set val at positions p
	for(tree[p += n] = val; p > 1; p >>= 1) tree[p >> 1] = tree[p] + tree[p ^ 1]; // p^1 gives sibling of p
}

lli query(lli l, lli r){ // oper. on [l, r] -< l,r are 1 based idx
	lli res = 0ll;
...
int main(void){
	...
	while(++tcase, t--){
		...
		lli q;
		cin >> n >> q;
		for(lli i = 0; i < n; ++i) cin >> tree[n + i];
		build();
		while(q--){
			lli nq, a, b;
			cin >> nq >> a >> b; 
			if(nq == 1) update(a - 1, b);
			else cout << query(a, b) << ln;
		}
