...
int main(void){
	...
	while(++tcase, t--){
		...
		if(f) cout << "NO SOLUTION\n";
			...
			string ans("");
			if(odd_char != '0') ans += string(1, odd_char);
			for(char ch = 'A'; ch <= 'Z'; ++ch){
				if(m[ch]){
					for(int j = 0; j < m[ch] / 2; ++j) ans += string(1, ch);
				}
			}
			string back;
			if(odd_char != '0') back = ans.substr(1, sz(ans) - 1);
			else back = ans;
			reverse(All(back));
			cout << back + ans << ln;
		}
