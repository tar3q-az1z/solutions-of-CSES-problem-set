...
int main(void){
	...
	while(++tcase, t--){
		...
		int n;
		cin >> n; 
		vector<string> ans(binpow(2, n), "");
		for(int i = 0; i < n; ++i){
			char ch = '0';
			int id = 0;
			for(int j = 0; j < binpow(2, n) / binpow(2, i); ++j){
				for(int k = 0; k < binpow(2, i); ++k){
					ans[id] += string(1, ch);
					++id;
				}
				if(id % binpow(2, i + 1)){
		...
		for(int i = 0; i < binpow(2, n); ++i) reverse(begin(ans[i]), end(ans[i]));
		for(int i = 0; i < binpow(2, n); ++i) cout << ans[i] << ln;
	}
	return 0;
}
