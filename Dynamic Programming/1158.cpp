...
int main(void){
	...
	while(++tcase, t--){
		...
		for(auto &i: h) cin >> i;
		for(auto &i: s) cin >> i;
		// state(i, j): i -> no of books, j -> price
		// dp[i][j] -> max page at j price and i no of books at most
		vector<vector<int>> dp(n + 1, vector<int>(x + 1, 0));
		// base case: dp[0][j] = 0; j -> [1, x], dp[i][0] = 0
		dp[0][0] = 0;
		/*
		STE:
		dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - Hi] + Xi)
				-> choose current book or not
		*/
		for(int i = 1; i <= n; ++i){
			for(int j = 0; j <= x; ++j){
				dp[i][j] = dp[i - 1][j];
		...
		// ans: dp[n][x] -> using at most x price and n books
		cout << dp[n][x] << ln;
		//_timer_();
	}
	return 0;
