...
int main(void){
	...
	while(++tcase, t--){
		...
		sort(begin(dur), end(dur), [](auto i, auto j){return i.second < j.second;});
		int prev_end = 0, ans = 0;
		for(auto &[k, v] : dur){
			if(prev_end <= k){   // previous end time must be less or equal than last start time
				++ans;
				prev_end = v;
			}
		}
