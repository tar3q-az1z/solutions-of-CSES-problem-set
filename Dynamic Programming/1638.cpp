...
int main(void){
	...
	while(++tcase, t--){
		...
		vector<string> a(n);
		for(auto &i: a) cin >> i;
		vector<vector<int>> dp(n + 1, vector<int>(n + 1, 0)); // state(r, c)
		if(a[0][0] == '.') dp[1][1] = 1; // one way for 1x1 grid; base case
		else dp[1][1] = 0;
		for(int i = 1; i <= n; ++i){
			for(int j = 1; j <= n; ++j){
				if(a[i - 1][j - 1] == '.'){
					/*
						STE: we either come from left or top at '.' position
						dp[i][j] = dp[i - 1][j] + dp[i][j - 1]
						(i, j) -> now
						(i - 1, j) -> top of (i, j) cell
						(i, j - 1) -> left of (i, j) cell
					*/
					if(j - 1 >= 1) dp[i][j] = (dp[i][j] + dp[i][j - 1]) % MOD.i;
					if(i - 1 >= 1) dp[i][j] = (dp[i][j] + dp[i - 1][j]) % MOD.i;
				}
			}
		}
		// ans: value at lower-right cell (n, n)
		cout << dp[n][n] << ln;
		//_timer_();
	}
	return 0;
